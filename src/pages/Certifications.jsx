...
const getIssuerIcon = (issuer) => {
  switch (issuer) {
    case 'Cisco': return <SiCisco className="text-4xl text-amber-400" />;
    case 'IBM': return <FaBriefcase className="text-4xl text-blue-400" />;
    case 'SAP': return <SiSap className="text-4xl text-blue-400" />;
    case 'Forage': return <FaBriefcase className="text-4xl text-amber-400" />;
              ...
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all duration-300 whitespace-nowrap border ${
                  isActive
                    ...
                    : 'bg-[#343a40] text-[#adb5bd] border-[#495057] hover:border-amber-400/60 hover:text-[#f8f9fa]'
                }`}>
                {tab}
        ...
        <motion.div key={activeTab} variants={containerVariants} initial="hidden" animate="visible"
          ...
          {filteredCerts.map(cert => (
            <motion.div key={cert.id} variants={itemVariants}
              className="bg-[#343a40] border border-[#495057] rounded-2xl p-6 flex flex-col h-full hover:border-amber-400/60 transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2 mt-6 pt-4 border-t border-[#495057]">
                {cert.certUrl && (
                  <a href={cert.certUrl} target="_blank" rel="noopener noreferrer"
                    className="flex-1 min-w-[100px] text-center bg-amber-400/10 hover:bg-amber-400/20 border border-amber-400/30 text-amber-400 text-xs font-medium py-2 px-3 rounded-lg flex justify-center items-center gap-1.5 transition-colors">
